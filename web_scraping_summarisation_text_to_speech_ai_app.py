...
def get_response(language: str, request: str, url: str, file: str,
    ...
    the summary to speech using a Text-to-Speech model.
    ...
    try:
        ...
        if language and any([url, file, text]):
            ...
            summarise the following text concisely without exceeding 100 
            tokens:

            Text: {text} 

            by respecting the user's request, if there is any: {request}.
            The summary language is {language} by default, without any user 
            specification."""
            ...
            input_tokens_length = (
                request_tokens_length + text_tokens_length +
                template_tokens_length
            )
